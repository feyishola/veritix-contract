...
impl VeritixToken {
    ...
    pub fn has_admin(e: Env) -> bool { has_admin(&e) }

    // --- Pause ---
    pub fn pause(e: Env, admin: Address) { check_admin(&e, &admin); pause(&e, admin); }
    pub fn unpause(e: Env, admin: Address) { check_admin(&e, &admin); unpause(&e, admin); }
    pub fn is_paused(e: Env) -> bool { is_paused(&e) }
    ...
    pub fn pause_recurring(e: Env, caller: Address, recurring_id: u32) { pause_recurring(&e, caller, recurring_id) }
    pub fn resume_recurring(e: Env, caller: Address, recurring_id: u32) { resume_recurring(&e, caller, recurring_id) }
    pub fn amend_recurring(e: Env, caller: Address, recurring_id: u32, new_amount: Option<i128>, new_interval: Option<u32>) { amend_recurring(&e, caller, recurring_id, new_amount, new_interval) }
    pub fn get_recurring(e: Env, recurring_id: u32) -> RecurringRecord { get_recurring(&e, recurring_id) }
    pub fn get_recurring_by_payer(e: Env, payer: Address) -> Vec<u32> { get_recurring_by_payer(&e, payer) }
    ...
    pub fn get_next_execution_ledger(e: Env, recurring_id: u32) -> u32 { get_next_execution_ledger(&e, recurring_id) }
    pub fn is_executable(e: Env, recurring_id: u32) -> bool { is_executable(&e, recurring_id) }
}
